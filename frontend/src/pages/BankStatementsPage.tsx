...
import {
    CheckCircle2, Download, FileText, Check,
    FileUp, BrainCircuit, AlertCircle, XCircle, X, Filter, Eye, List, Loader2, FileSpreadsheet,
    Landmark
} from 'lucide-react';
...
export default function BankStatementsPage() {
    ...
    const handleDelete = (id: string) => {
        if (confirm(t('bankStatements.deleteConfirm'))) {
            setDeletingId(id);
            deleteMut.mutate(id);
        }
    };
    ...
    const toggleColumn = (col: ColKey) => {
        setVisibleCols(prev => {
            ...
            const nsKey = getNamespacedKey('bank_statements_visible_cols', true);
    ...
    const handlePreview = async (id: string) => {
        ...
        try {
            ...
            alert(t('bankStatements.errors.previewFailed'));
        } finally {
        ...
        <div className="space-y-6 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                    <Landmark className="text-indigo-600 dark:text-indigo-400" /> {t('bankStatements.title')}
                </h1>
            </div>
            ...
            <BankStatementTable
                ...
                onSort={handleSort}
                onNavigateTransactions={(id) => navigate(`/transactions?statement=${id}`)}
                onPreview={handlePreview}
                onView={setViewingStatement}
                    ...
                    {data.url ? (
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white dark:bg-gray-900 rounded-xl border border-gray-300 dark:border-gray-800 shadow-inner">
                            ...
                            <h4 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
                                {t('bankStatements.modal.previewNotSupportedTitle')}
                            </h4>
                            <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm mb-6">
                                {t('bankStatements.modal.previewNotSupportedDesc')}
                            </p>
                            <button
                            ...
                            >
                                <Download size={18} />
                                {t('bankStatements.actions.download')}
                            </button>
                        </div>
                    )}
                </div>
