...
export default function ShareInvoiceModal({ invoice, onClose }: ShareInvoiceModalProps) {
    ...
    const { data: sharedUserIds = [], isLoading: isLoadingShares } = useQuery({
        queryKey: ['invoice-shares', invoice.id],
        queryFn: () => invoiceService.fetchShares(invoice.id),
    });

    const shareMut = useMutation({
        mutationFn: ({ userId, perm }: { userId: string; perm: 'view' | 'edit' }) => 
            invoiceService.share(invoice.id, userId, perm),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['invoice-shares', invoice.id] });
    ...
    const revokeMut = useMutation({
        mutationFn: (userId: string) => invoiceService.revokeShare(invoice.id, userId),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['invoice-shares', invoice.id] });
            qc.invalidateQueries({ queryKey: ['invoices'] });
        },
    ...
    const availableUsers = users.filter(
        u => u.id !== invoice.owner_id && !sharedUserIds.includes(u.id)
    );
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                        ...
                        {t('invoices.shareTitle', { vendor: invoice.vendor.name })}
                    </h3>
                    <button
                        onClick={onClose}
                    ...
                    <div className="space-y-4 pt-4">
                        ...
                        {isLoadingShares ? (
                            ...
                            <div className="text-center py-6 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-2xl">
                                <p className="text-sm text-gray-400">{t('invoices.noParticipants')}</p>
                            </div>
                        ) : (
