...
func (m *mockCategorizer) CategorizeBatch(ctx context.Context, txns []port.TransactionToCategorize, categories []string) ([]port.CategorizedTransaction, error) {
	m.calls++
	if m.err != nil {
		return nil, m.err
	}
...
func TestAnalytics_ExcludesReconciledTx(t *testing.T) {
	...
	svc := service.NewTransactionService(repo, catRepo, nil, setupLogger())
...
func TestStartAutoCategorizeAsync_Success(t *testing.T) {
	...
	svc := service.NewTransactionService(repo, catRepo, llm, setupLogger())
