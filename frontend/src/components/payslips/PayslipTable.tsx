            ...
            <div className="w-full overflow-x-auto pb-2">
                <table className="min-w-[1000px] w-full divide-y divide-gray-100 dark:divide-gray-800/50 text-sm">
                    ...
                    <tr>
                        ...
                        {visibleCols.employee && <th onClick={() => onSort('employee')} className="px-5 py-3.5 text-left font-medium cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 group/th"><div className="flex items-center gap-1.5">{t('payslips.table.employee')} {renderSortIcon('employee')}</div></th>}
                        ...
                        filteredPayslips.map((p) => {
                            ...
                            return (
                                <tr key={p.id} className={`transition-colors group ${isIgnored ? 'bg-gray-50/50 dark:bg-gray-800/20 opacity-60 hover:opacity-100' : 'hover:bg-gray-50 dark:hover:bg-gray-800/50'}`}>
                                    ...
                                    {visibleCols.employee && <td className="px-5 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">{p.employee_name}</td>}
                                    {visibleCols.employer && <td className="px-5 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">{p.employer_name}</td>}
                                    {visibleCols.gross && <td className="px-5 py-3 text-gray-500 dark:text-gray-400 text-right whitespace-nowrap font-mono">{fmtCurrency(p.gross_pay, 'EUR')}</td>}
                                    {visibleCols.net && <td className="px-5 py-3 font-medium text-gray-900 dark:text-gray-100 text-right whitespace-nowrap font-mono">{fmtCurrency(p.net_pay, 'EUR')}</td>}
                                    ...
                                    {visibleCols.payout && <td className="px-5 py-3 text-gray-500 dark:text-gray-400 text-right whitespace-nowrap font-mono">{fmtCurrency(p.payout_amount, 'EUR')}</td>}
                                    {visibleCols.leasing && <td className="px-5 py-3 text-gray-500 dark:text-gray-400 text-right whitespace-nowrap font-mono">{fmtCurrency(p.custom_deductions, 'EUR')}</td>}
                                    <td className="px-5 py-3 whitespace-nowrap text-right">
                                        <div className="flex items-center justify-end gap-2 opacity-100 lg:opacity-40 lg:group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button onClick={() => onPreview(p.id)} disabled={isPreviewLoading === p.id || !p.original_file_mime} className="p-1.5 text-gray-400 hover:text-fuchsia-600 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-900/30 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed" title={p.original_file_mime ? t('payslips.table.previewPdf') : t('payslips.table.noFile')}>
                                            ...
                                            <button onClick={() => onDownload(p.id, p.original_file_name)} disabled={!p.original_file_mime} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed" title={p.original_file_mime ? t('payslips.table.downloadPdf') : t('payslips.table.noFile')}><Download size={18} /></button>
