...
export const categorizeDocument = (rawText: string): Promise<Invoice> =>
    api
        .post<Invoice>('/invoices/categorize', {raw_text: rawText})
...
export const fetchBankStatementBlob = async (id: string): Promise<Blob> => {
    // We use the 'api' instance here so the Authorization header is
    // automatically attached via your interceptor.
    // Note: baseURL is already '/api/v1' so we just use the relative path
...
// statementId wird nicht mehr benötigt
...
export const getPayslipPreviewUrl = async (id: string): Promise<string> => {
    ...
    // Explicitly cast to application/pdf so the browser knows to render it inline
...
export const importPayslip = async ({file, overrides, useAI}: {
    ...
    // Add the force AI flag if checked
    if (useAI) {
...
export const updatePayslip = (id: string, data: Partial<Payslip> | FormData): Promise<Payslip> => {
    ...
    // The backend uses PUT for updates
