...
type InMemorySharingRepository struct {
	...
	accountShares  map[uuid.UUID]map[uuid.UUID]shareRecord // map[accountID]map[sharedWithID]record
}
...
func NewInMemorySharingRepository() *InMemorySharingRepository {
	return &InMemorySharingRepository{
		...
		accountShares:  make(map[uuid.UUID]map[uuid.UUID]shareRecord),
	}
}
...
func (r *InMemorySharingRepository) ShareBankAccount(ctx context.Context, bankAccountID, ownerID, sharedWithID uuid.UUID, permission string) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	if r.accountShares[bankAccountID] == nil {
		r.accountShares[bankAccountID] = make(map[uuid.UUID]shareRecord)
	}
	r.accountShares[bankAccountID][sharedWithID] = shareRecord{
		OwnerID:      ownerID,
		SharedWithID: sharedWithID,
		Permission:   permission,
	}
...
func (r *InMemorySharingRepository) RevokeBankAccountShare(ctx context.Context, bankAccountID, ownerID, sharedWithID uuid.UUID) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	shares, exists := r.accountShares[bankAccountID]
	if !exists {
		return nil // idempotent
	}
	...
	delete(r.accountShares[bankAccountID], sharedWithID)
	return nil
}

func (r *InMemorySharingRepository) ListBankAccountShares(ctx context.Context, bankAccountID, ownerID uuid.UUID) ([]uuid.UUID, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func TestInMemorySharingRepository_BankAccountShares(t *testing.T) {
	ctx := context.Background()
	repo := NewInMemorySharingRepository()
	...
	t.Run("Share Bank Account", func(t *testing.T) {
		err := repo.ShareBankAccount(ctx, accID, ownerID, partner, "view")
		require.NoError(t, err)
	})

	t.Run("List Bank Account Shares", func(t *testing.T) {
		shares, err := repo.ListBankAccountShares(ctx, accID, ownerID)
		require.NoError(t, err)
		require.Len(t, shares, 1)
		assert.Equal(t, partner, shares[0])
	})

	t.Run("Revoke Bank Account Share", func(t *testing.T) {
		err := repo.RevokeBankAccountShare(ctx, accID, ownerID, partner)
		require.NoError(t, err)
