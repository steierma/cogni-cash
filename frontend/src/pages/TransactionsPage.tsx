...
import {
    ArrowLeft, BarChart3, Database, Layers, Loader2, Search,
    Sparkles, TrendingDown, TrendingUp, Trophy, Unlink, X
} from 'lucide-react';
...
export default function TransactionsPage() {
    ...
    // If we land here with a statement via URL, immediately flag as applied so the list fetches.
    const [hasAppliedOnce, setHasAppliedOnce] = React.useState(!!preselectedStatement);

    // Automatically reveal reconciled items when jumping to a specific statement.
    ...
    const filtered: Transaction[] = React.useMemo(() => {
        ...
        if (hideReconciled) {
            rows = rows.filter((t) => {
                ...
                if (t.statement_type === 'credit_card' && t.amount > 0) return false;
                return true;
            });
        }
        ...
        if (applied.search.trim()) {
            ...
            rows = rows.filter((t) => t.description.toLowerCase().includes(q) || t.reference.toLowerCase().includes(q));
        }
        ...
        if (applied.amountMin !== '') {
            rows = rows.filter((t) => t.amount >= parseAmount(applied.amountMin));
        }
        if (applied.amountMax !== '') {
            rows = rows.filter((t) => t.amount <= parseAmount(applied.amountMax));
        }
        ...
        <div className="max-w-7xl mx-auto space-y-6 pb-20">
            ...
            {!isLoading && !hasAppliedOnce && (
                <div className="flex flex-col items-center justify-center py-32 bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-gray-200 dark:border-gray-800 text-gray-400 dark:text-gray-500">
                    ...
                    <p className="text-base font-medium text-gray-600 dark:text-gray-300">No data loaded yet</p>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-6 flex flex-col">
                                ...
                                {chartData.monthly.length > 0 ? (
                                    ...
                                    <div className="flex-1 flex items-center justify-center text-sm text-gray-400 dark:text-gray-500">No timeline data available.</div>
                                )}
                            </div>
                                    ...
                                    <div key={tx.content_hash} className="flex justify-between items-center bg-red-50/40 dark:bg-red-900/10 hover:bg-red-50/80 dark:hover:bg-red-900/20 transition-colors p-2.5 rounded-lg border border-red-100/50 dark:border-red-900/30">
                                        <div className="min-w-0 flex-1 pr-3">
                                            ...
                                            <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">
                                                {fmtDate(tx.booking_date)} • {categories.find(c => c.id === tx.category_id)?.name || 'Uncategorized'}
                                            </p>
                                        </div>
                ...
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <div className="bg-gray-900 dark:bg-gray-800 text-white rounded-2xl shadow-2xl px-6 py-4 flex items-center gap-6 border border-white/10 dark:border-gray-700">
                        <div className="flex items-center gap-2 pr-4 border-r border-gray-700 dark:border-gray-600">
                            ...
                            <span className="font-bold text-sm">{t('transactions.selected', { count: selectedHashes.size })}</span>
                        </div>

                        <div className="flex items-center gap-3">
                            <span className="text-xs text-gray-400 font-medium uppercase tracking-wider">{t('transactions.setCategory')}</span>
                            <select
                        ...
                        <button onClick={() => setSelectedHashes(new Set())} className="text-gray-400 hover:text-white transition-colors">
                            <X size={20} />
                        </button>
                    </div>
                </div>
