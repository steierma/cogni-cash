...
type TransactionUseCase interface {
	...
	UpdateCategory(ctx context.Context, hash string, categoryID *uuid.UUID) error
	MarkAsReviewed(ctx context.Context, hash string) error
...
type BankUseCase interface {
	// Connections
	GetInstitutions(ctx context.Context, countryCode string, isSandbox bool) ([]entity.BankInstitution, error)                                                        // HIER ANGEPASST
	CreateConnection(ctx context.Context, userID uuid.UUID, institutionID string, country string, redirectURL string, isSandbox bool) (*entity.BankConnection, error) // HIER ANGEPASST
	FinishConnection(ctx context.Context, requisitionID string, code string) error
	...
	UpdateAccountType(ctx context.Context, accountID uuid.UUID, accType entity.StatementType) error
}
