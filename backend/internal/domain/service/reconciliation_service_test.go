...
func TestReconcileStatements_SameAccountType(t *testing.T) {
...
func TestReconciliationService_SuggestReconciliations_SkipsSameStatementType(t *testing.T) {
	// Both are giro — should not match
	repo := &mockRepo{
		existingTxns: []entity.Transaction{
			{BookingDate: time.Date(2026, 3, 10, 0, 0, 0, 0, time.UTC), Amount: -200, ContentHash: "g1", StatementType: entity.StatementTypeGiro},
			{BookingDate: time.Date(2026, 3, 11, 0, 0, 0, 0, time.UTC), Amount: 200, ContentHash: "g2", StatementType: entity.StatementTypeGiro},
		},
	}
	svc := service.NewReconciliationService(repo, nil, setupLogger())
