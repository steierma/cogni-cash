...
BEGIN
    ...
    INSERT INTO categories (name, color, user_id) VALUES ('Tech & Software', '#3b82f6', admin_id)
    ON CONFLICT (name, user_id) DO UPDATE SET name=EXCLUDED.name RETURNING id INTO cat_tech;

    INSERT INTO categories (name, color, user_id) VALUES ('Groceries & Food', '#10b981', admin_id)
    ON CONFLICT (name, user_id) DO UPDATE SET name=EXCLUDED.name RETURNING id INTO cat_groceries;

    INSERT INTO categories (name, color, user_id) VALUES ('Utilities & Internet', '#f97316', admin_id)
    ...
    WHILE curr_date <= end_date LOOP
        ...
        -- ---------------------------------------------------------
        -- 1. Income (Einkommen)
        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_giro_id, curr_date + interval '1 day', curr_date + interval '1 day', 'Salary Mustermann GmbH', salary_net, 'credit', cat_income, md5(gen_random_uuid()::text), false, 'giro', true);

        -- 2. Rent (Haus und Hausrat)
        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_giro_id, curr_date + interval '3 days', curr_date + interval '3 days', 'Rent Payment', -1200.00, 'debit', cat_housing, md5(gen_random_uuid()::text), false, 'giro', true);

        -- 3. Utilities (Utilities & Internet)
        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_giro_id, curr_date + interval '4 days', curr_date + interval '4 days', 'Telekom Internet & Power', -utilities_amount, 'debit', cat_utilities, md5(gen_random_uuid()::text), false, 'giro', true);

        -- 4. Groceries (Groceries & Food)
        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_giro_id, curr_date + interval '10 days', curr_date + interval '10 days', 'REWE Supermarket', -groceries_amount, 'debit', cat_groceries, md5(gen_random_uuid()::text), false, 'giro', true);

        -- 5. Tech & Subscriptions (Tech & Software)
        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_giro_id, curr_date + interval '12 days', curr_date + interval '12 days', 'Hetzner Online GmbH', -invoice_amount, 'debit', cat_tech, md5(gen_random_uuid()::text), false, 'giro', true);
        ...
        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_giro_id, curr_date + interval '15 days', curr_date + interval '15 days', 'Internal Transfer to Savings', -recon_amount, 'debit', cat_misc, md5(gen_random_uuid()::text), false, 'giro', true);

        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_savings_id, curr_date + interval '16 days', curr_date + interval '16 days', 'Internal Transfer from Giro', recon_amount, 'credit', cat_misc, md5(gen_random_uuid()::text), false, 'credit_card', true);

        -- Pair B: Guaranteed same-day exact match (1500.00)
        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_giro_id, curr_date + interval '20 days', curr_date + interval '20 days', 'Manual Transfer Extra (Out)', -1500.00, 'debit', cat_misc, md5(gen_random_uuid()::text), false, 'giro', true);

        INSERT INTO transactions (user_id, bank_statement_id, booking_date, valuta_date, description, amount, transaction_type, category_id, content_hash, is_reconciled, statement_type, reviewed)
        VALUES (admin_id, stmt_savings_id, curr_date + interval '20 days', curr_date + interval '20 days', 'Manual Transfer Extra (In)', 1500.00, 'credit', cat_misc, md5(gen_random_uuid()::text), false, 'credit_card', true);

        -- ---------------------------------------------------------
        -- Standalone Invoices & Payslips
        -- ---------------------------------------------------------
        INSERT INTO invoices (user_id, raw_text, vendor, amount, invoice_date, category_id)
        VALUES (admin_id, 'Hetzner Online GmbH Cloud Server Instance...', 'Hetzner Online GmbH', invoice_amount, curr_date + interval '5 days', cat_tech);

        INSERT INTO payslips (user_id, employer_name, original_file_name, original_file_size, content_hash, period_month_num, period_year, employee_name, tax_class, tax_id, gross_pay, net_pay, payout_amount)
        VALUES (
            admin_id,
            'Mustermann GmbH',
