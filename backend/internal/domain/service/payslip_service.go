...
type PayslipParserPort interface {
	Parse(ctx context.Context, filePath string) (entity.Payslip, error)
}

type PayslipService struct {
	repo         port.PayslipRepository
	staticParser PayslipParserPort
	aiParser     PayslipParserPort
	logger       *slog.Logger
}

func NewPayslipService(repo port.PayslipRepository, staticParser PayslipParserPort, aiParser PayslipParserPort, logger *slog.Logger) *PayslipService {
