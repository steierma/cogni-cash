...
export default function ShareCategoryModal({ category, onClose }: ShareCategoryModalProps) {
    ...
    const { data: sharedUserIds = [], isLoading: isLoadingShares } = useQuery({
        ...
        queryFn: () => categoryService.fetchShares(category.id),
    });

    const shareMut = useMutation({
        mutationFn: ({ userId, perm }: { userId: string; perm: 'view' | 'edit' }) => 
            categoryService.share(category.id, userId, perm),
        onSuccess: () => {
    ...
    const revokeMut = useMutation({
        mutationFn: (userId: string) => categoryService.revokeShare(category.id, userId),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['category-shares', category.id] });
            qc.invalidateQueries({ queryKey: ['categories'] });
        },
    ...
    const availableUsers = users.filter(
        u => u.id !== category.owner_id && !sharedUserIds.includes(u.id)
    );
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                        ...
                        {t('categories.shareTitle', { name: category.name })}
                    </h3>
                    <button
                        onClick={onClose}
