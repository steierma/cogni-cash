...
func main() {
	...
	if storageMode == "memory" {
		...
		invoiceRepo = pgrepo.NewInvoiceRepository(pool, vaultKey, logger)
		bankStmtRepo = pgrepo.NewBankStatementRepository(pool, vaultKey, logger)
		...
		payslipRepo = pgrepo.NewPayslipRepository(pool, vaultKey)
	...
	// --- HTTP Handler Setup ---
	router := httpAdapter.NewRouter(logger, appCtx, &wg, authSvc, userSvc, bridgeTokenSvc)
	router.Auth = httpAdapter.NewAuthHandler(logger, authSvc, bridgeTokenSvc, userSvc)
	router.Invoice = httpAdapter.NewInvoiceHandler(logger, invoiceSvc)
	router.BankStatement = httpAdapter.NewBankStatementHandler(logger, bankStatementSvc, bankStmtRepo, forecastingSvc, settingsSvc, transactionSvc)
	router.Reconciliation = httpAdapter.NewReconciliationHandler(logger, reconciliationRepo, reconciliationSvc)
	router.Category = httpAdapter.NewCategoryHandler(categorySvc, forecastingSvc)
	router.Payslip = httpAdapter.NewPayslipHandler(logger, payslipRepo, payslipSvc)
	router.Bank = httpAdapter.NewBankHandler(appCtx, logger, &wg, bankSvc)
	router.Forecasting = httpAdapter.NewForecastingHandler(logger, forecastingSvc)
	router.User = httpAdapter.NewUserHandler(appCtx, logger, &wg, notificationSvc, userSvc)
	router.PlannedTransaction = httpAdapter.NewPlannedTransactionHandler(logger, plannedTransactionSvc)
	router.Sharing = httpAdapter.NewSharingHandler(logger, sharingSvc)
	router.Discovery = httpAdapter.NewDiscoveryHandler(appCtx, logger, &wg, discoverySvc)
	router.BridgeToken = httpAdapter.NewBridgeTokenHandler(logger, bridgeTokenSvc)
	router.Document = httpAdapter.NewDocumentHandler(logger, documentSvc)
	router.System = httpAdapter.NewSystemHandler(logLevelVar, logger, dbHost, dbPinger, notificationSvc, settingsSvc, storageMode, userSvc)

	srv := httpAdapter.NewServer(addr, router)
