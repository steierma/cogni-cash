...
var _ port.CategorizationUseCase = (*TransactionService)(nil)
...
func NewTransactionService(repo port.BankStatementRepository, catRepo port.CategoryRepository, llm port.TransactionCategorizer, logger *slog.Logger) *TransactionService {
	if logger == nil {
		logger = slog.Default()
	}
...
func (s *TransactionService) StartAutoCategorizeAsync(ctx context.Context, batchSize int) error {
	...
	go s.runCategorizeLoop(jobCtx, toCategorize, cats, batchSize)

	return nil
}

func (s *TransactionService) runCategorizeLoop(ctx context.Context, txns []port.TransactionToCategorize, categories []entity.Category, batchSize int) {
	s.Logger.Info("Starting auto-categorization job", "total_transactions", len(txns))
	defer func() {
	...
	for i := 0; i < len(txns); i += batchSize {
		...
		results, err := s.llm.CategorizeBatch(ctx, batch, catNames)
		if err != nil {
			if ctx.Err() != nil {
				s.JobTracker.Finish("cancelled")
			} else {
