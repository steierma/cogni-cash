...
func (h *Handler) getMe(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		h.Logger.Warn("getMe failed: missing or invalid userIDKey in context")
	...
	if err != nil {
		h.Logger.Error("getMe failed: could not fetch profile", "user_id", userIDStr, "error", err)
		writeError(w, http.StatusNotFound, "user profile not found")
		return
	}

	h.Logger.Info("getMe successful", "user_id", userIDStr, "username", user.Username)
