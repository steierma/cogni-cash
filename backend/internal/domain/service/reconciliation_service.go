...
var ErrTransactionNotFound = errors.New("reconciliation service: transaction not found")
var ErrSameAccount = errors.New("reconciliation service: source and target must be from different accounts")

type ReconciliationPairSuggestion struct {
	SourceTransaction entity.Transaction `json:"source_transaction"`
	TargetTransaction entity.Transaction `json:"target_transaction"`
...
func (s *ReconciliationService) ReconcileStatements(
	...
	// Prevent reconciling two transactions from the same account type.
	if settlementTx.StatementType == targetTx.StatementType {
...
func (s *ReconciliationService) SuggestReconciliations(ctx context.Context, matchWindowDays int) ([]ReconciliationPairSuggestion, error) {
	...
	var suggestions []ReconciliationPairSuggestion
	...
	for _, debit := range debits {
		...
		for _, credit := range credits {
			...
			// Skip if both transactions are from the same account type.
			if debit.StatementType == credit.StatementType {
				continue
			}

			// Skip if the target (credit) date is before the source (debit) date.
			// A settlement payment always debits first; the credit appears later.
			...
			if math.Abs(debit.Amount+credit.Amount) < 0.01 {
				...
				if diffHours <= float64(24*matchWindowDays) {
					suggestions = append(suggestions, ReconciliationPairSuggestion{
