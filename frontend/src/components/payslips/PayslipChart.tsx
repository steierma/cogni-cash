...
interface PayslipChartProps {
    ...
    setExcludeLeasing: (val: boolean) => void;
...
export function PayslipChart({
                                 ...
                                 excludeLeasing, setExcludeLeasing, useProportionalMath, setUseProportionalMath
                             }: PayslipChartProps) {
    ...
    const chartData = useMemo(() => {
        ...
        const baselineAdjNet = getAdjustedNetto(baselinePayslip, excludedBonuses, excludeLeasing, useProportionalMath);
        ...
        return sortedForChart.map((p) => {
            ...
            const adjNet = getAdjustedNetto(p, excludedBonuses, excludeLeasing, useProportionalMath);
    ...
    }, [filteredPayslips, excludedBonuses, excludeLeasing, useProportionalMath, ignoredPayslipIds]);
        ...
        <div className="bg-white dark:bg-gray-900 p-5 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm flex flex-col gap-4">
            <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-4 mb-2">
                <div className="flex items-start gap-2">
                    <LineChartIcon className="text-indigo-500 mt-0.5" size={20} />
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('payslips.chart.title')}</h3>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-2 xl:mt-0">
                    <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700">
                        ...
                        <label className="flex items-center gap-1.5 text-xs font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-gray-900 dark:hover:text-gray-100">
                            <input type="checkbox" checked={excludeLeasing} onChange={e => setExcludeLeasing(e.target.checked)} className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 bg-white dark:bg-gray-900" />
                            {t('payslips.chart.excludeLeasing')}
                        </label>
                        <div className="w-px h-4 bg-gray-300 dark:bg-gray-600"></div>
