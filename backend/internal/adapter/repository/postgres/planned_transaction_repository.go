...
func (r *PlannedTransactionRepository) Create(ctx context.Context, pt *entity.PlannedTransaction) error {
	...
	err := r.pool.QueryRow(ctx, `
		INSERT INTO planned_transactions (id, user_id, amount, date, description, category_id, status, matched_transaction_id)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		RETURNING created_at`,
		pt.ID, pt.UserID, pt.Amount, pt.Date, pt.Description, pt.CategoryID, pt.Status, pt.MatchedTransactionID).
		Scan(&pt.CreatedAt)
	if err != nil {
...
func (r *PlannedTransactionRepository) GetByID(ctx context.Context, id uuid.UUID, userID uuid.UUID) (*entity.PlannedTransaction, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT id, user_id, amount, date, description, category_id, status, matched_transaction_id, created_at
		FROM planned_transactions
		WHERE id = $1 AND user_id = $2`, id, userID).
		Scan(&pt.ID, &pt.UserID, &pt.Amount, &pt.Date, &pt.Description, &pt.CategoryID, &pt.Status, &pt.MatchedTransactionID, &pt.CreatedAt)
	if err != nil {
...
func (r *PlannedTransactionRepository) Update(ctx context.Context, pt *entity.PlannedTransaction) error {
	...
	tag, err := r.pool.Exec(ctx, `
		...
		SET amount = $1, date = $2, description = $3, category_id = $4, status = $5, matched_transaction_id = $6
		WHERE id = $7 AND user_id = $8`,
		pt.Amount, pt.Date, pt.Description, pt.CategoryID, pt.Status, pt.MatchedTransactionID, pt.ID, pt.UserID)
	if err != nil {
...
func (r *PlannedTransactionRepository) FindByUserID(ctx context.Context, userID uuid.UUID) ([]entity.PlannedTransaction, error) {
	...
	for rows.Next() {
		var pt entity.PlannedTransaction
		if err := rows.Scan(&pt.ID, &pt.UserID, &pt.Amount, &pt.Date, &pt.Description, &pt.CategoryID, &pt.Status, &pt.MatchedTransactionID, &pt.CreatedAt); err != nil {
